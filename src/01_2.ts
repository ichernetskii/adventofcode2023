const input = `sdpgz3five4seven6fiveh
876mbxbrntsfm
...
const DIGITS = Object.keys(DIGITS_MAP);

const lines = input.split("\n").map(line => line.trim())
    .filter(line => line.length > 0);

const isDigit = (ch: string): boolean => {
    return !isNaN(parseInt(ch)) && ch.length === 1;
}

const firstLastNumbers = lines.map(line => {
    ...
    for (const char of charArr) {
        if (isDigit(char)) {
            ...
            for (const digit of DIGITS) {
                if (currentString.includes(digit)) {
                    numbers.push(DIGITS_MAP[digit]);
                    currentString = "";
                    break;
                }
            }
    ...
    currentString = "";
    const numbersEnd: number[] = [];
    const reversedCharArr = line.split("").reverse();
    for (const char of reversedCharArr) {
        if (isDigit(char)) {
            currentString = "";
            numbersEnd.push(parseInt(char));
        } else {
            currentString = char + currentString;

            for (const digit of DIGITS) {
                if (currentString.includes(digit)) {
                    numbersEnd.push(DIGITS_MAP[digit]);
                    currentString = "";
                    break;
                }
            }
    ...
    return 10 * numbers[0] + numbersEnd[0];
});

const result = firstLastNumbers.reduce((acc, curr) => acc + curr, 0);
